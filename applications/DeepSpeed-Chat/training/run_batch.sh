...
for hostfile in "$hostfiles_folder_path"/hostfile_*; do
  if [ -f "$hostfile" ]; then
    ...
    cmd_job="export PROJECT_PATH=${PROJECT_PATH} && cd ${PROJECT_PATH}/DeepSpeedExamples/applications/DeepSpeed-Chat/training && timeout ${timeout_seconds}s ./${script_name}.sh $job_name $hostfiles_folder_path/$filename $output_folder_path"
    
    # Submit the job to the login node in parallel
    echo $cmd_job
    ssh -t "$login_node" "$cmd_job" &
  fi
done
