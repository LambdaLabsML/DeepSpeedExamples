...
for hostfile in "$hostfiles_folder_path"/hostfile_*; do
  if [ -f "$hostfile" ]; then
    ...
    cmd_job="cd ~/ml/DeepSpeedExamples/applications/DeepSpeed-Chat/training && timeout ${timeout_seconds}s ./${script_name}.sh $job_name $hostfiles_folder_path/$filename $output_folder_path"
