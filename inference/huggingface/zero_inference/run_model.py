...
def get_tokenizer(model_name, config):
    if config.model_type == "opt":
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            model_name, return_token_type_ids=False
        )
...
def run_generation(
    ...
    # Run
    print(
        f"benchmark, prompt_len = {prompt_len}, execute_gen_len = {execute_gen_len}, input_ids.shape = {input_tokens.input_ids.shape}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="model name or path; currently only supports OPT and BLOOM models",
    )
    parser.add_argument(
        "--dummy", action="store_true", help="Use dummy weights for benchmark purposes."
    )
    parser.add_argument(
        "--loops", type=int, default=3, help="Number of token generation iterations"
    )
    parser.add_argument("--batch-size", type=int, default=1)
    parser.add_argument("--prompt-len", type=int, default=512, help="prompt length")
    parser.add_argument(
        "--gen-len", type=int, default=32, help="number of tokens to generate"
    )
    parser.add_argument(
        "--local_rank", type=int, help="local rank for distributed inference"
    )
    parser.add_argument(
        "--pin-memory",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="~/offload_dir",
        help="Directory to store offloaded cache.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pin_kv_cache",
        action="store_true",
        help="Allocate kv cache in pinned memory for offloading.",
    )
    parser.add_argument(
        "--async_kv_offload",
        action="store_true",
        help="Using non_blocking copy for kv cache offloading.",
    )
